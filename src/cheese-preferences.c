/* cheese-preferences.c generated by valac 0.39.7.25-e3fff, the Vala compiler
            ...
            CheeseVideoFormat* format)
{
	...
	_tmp8_ = _tmp7_;
	_tmp9_ = format->height;
	_tmp10_ = g_strdup_printf ("%i", _tmp9_);
	_tmp11_ = _tmp10_;
	_tmp12_ = g_strconcat (_tmp8_, _tmp11_, NULL);
	_tmp13_ = _tmp12_;
	gtk_list_store_set (_tmp2_, &_tmp3_, 0, _tmp13_, 1, format, -1);
	_g_free0 (_tmp13_);
	_g_free0 (_tmp11_);
	_g_free0 (_tmp8_);
	_g_free0 (_tmp6_);
	...
	_tmp18_ = format->width;
	if (_tmp17_ == _tmp18_) {
		CheeseCamera* _tmp19_;
		CheeseVideoFormat* _tmp20_;
		gint _tmp21_;
		gint _tmp22_;
		_tmp19_ = self->priv->camera;
		_tmp20_ = cheese_camera_get_current_video_format (_tmp19_);
		_tmp21_ = _tmp20_->height;
		_tmp22_ = format->height;
	...
	if (_tmp14_) {
		...
		GSettings* _tmp27_;
		gint _tmp28_;
		_tmp23_ = self->priv->photo_resolution_combo;
		_tmp24_ = iter;
		gtk_combo_box_set_active_iter (_tmp23_, &_tmp24_);
		_tmp25_ = self->priv->settings;
		_tmp26_ = format->width;
		g_settings_set_int (_tmp25_, "photo-x-resolution", _tmp26_);
		_tmp27_ = self->priv->settings;
		_tmp28_ = format->height;
		g_settings_set_int (_tmp27_, "photo-y-resolution", _tmp28_);
	}
	_tmp30_ = self->priv->settings;
	_tmp31_ = format->width;
	if (g_settings_get_int (_tmp30_, "video-x-resolution") == _tmp31_) {
		GSettings* _tmp32_;
		gint _tmp33_;
		_tmp32_ = self->priv->settings;
		_tmp33_ = format->height;
		_tmp29_ = g_settings_get_int (_tmp32_, "video-y-resolution") == _tmp33_;
	} else {
		_tmp29_ = FALSE;
	}
	if (_tmp29_) {
		GtkComboBox* _tmp34_;
		GtkTreeIter _tmp35_;
		_tmp34_ = self->priv->video_resolution_combo;
		_tmp35_ = iter;
                                            ...
                                            GtkComboBox* combo)
{
	...
	_tmp5_ = _tmp4_;
	_tmp6_ = iter;
	gtk_tree_model_get (_tmp5_, &_tmp6_, 1, &dev, -1);
	_tmp7_ = self->priv->camera;
	cheese_camera_set_device (_tmp7_, dev);
	...
	_tmp11_ = self->priv->settings;
	_tmp12_ = cheese_camera_device_get_name (dev);
                                                      ...
                                                      GtkComboBox* combo)
{
	...
	_tmp2_ = _tmp1_;
	_tmp3_ = iter;
	gtk_tree_model_get (_tmp2_, &_tmp3_, 1, &format, -1);
	_tmp5_ = self->priv->current_mode;
	if (_tmp5_ == CHEESE_MEDIA_MODE_PHOTO) {
		_tmp4_ = TRUE;
	} else {
		CheeseMediaMode _tmp6_;
		_tmp6_ = self->priv->current_mode;
		_tmp4_ = _tmp6_ == CHEESE_MEDIA_MODE_BURST;
	}
	if (_tmp4_) {
		CheeseCamera* _tmp7_;
	...
	g_settings_set_int (_tmp9_, "photo-x-resolution", _tmp11_);
	_tmp12_ = self->priv->settings;
	_tmp13_ = format;
	_tmp14_ = _tmp13_->height;
	g_settings_set_int (_tmp12_, "photo-y-resolution", _tmp14_);
                                                      ...
                                                      GtkComboBox* combo)
{
	...
	if (_tmp4_ == CHEESE_MEDIA_MODE_VIDEO) {
		CheeseCamera* _tmp5_;
	...
	_tmp8_ = format;
	_tmp9_ = _tmp8_->width;
	g_settings_set_int (_tmp7_, "video-x-resolution", _tmp9_);
	_tmp10_ = self->priv->settings;
	_tmp11_ = format;
	_tmp12_ = _tmp11_->height;
	g_settings_set_int (_tmp10_, "video-y-resolution", _tmp12_);
	__vala_CheeseVideoFormat_free0 (format);
}
             ...
             void* device)
{
	...
	GtkListStore* _tmp8_;
	self = _data2_->self;
	_tmp0_ = _g_object_ref0 (G_TYPE_CHECK_INSTANCE_CAST (device, CHEESE_TYPE_CAMERA_DEVICE, CheeseCameraDevice));
	old_device = _tmp0_;
	_tmp1_ = self->priv->camera_model;
	_tmp2_ = _data2_->iter;
	gtk_tree_model_get ((GtkTreeModel*) _tmp1_, &_tmp2_, 1, &new_device, -1, -1);
	_tmp3_ = old_device;
	_tmp4_ = new_device;
	if (_tmp3_ != _tmp4_) {
		GtkTreeIter _tmp5_;
		...
		_tmp5_ = _data2_->iter;
		_tmp6_ = new_device;
		_tmp7_ = _data2_->active_device;
		cheese_preferences_dialog_remove_camera_device (self, &_tmp5_, _tmp6_, _tmp7_);
	...
	gtk_tree_model_iter_next ((GtkTreeModel*) _tmp8_, &_data2_->iter);
	_g_object_unref0 (new_device);
	_g_object_unref0 (old_device);
}
...
cheese_preferences_dialog_on_camera_update_num_camera_devices (CheesePreferencesDialog* self)
{
	...
	_tmp3_ = _tmp2_->len;
	_tmp4_ = self->priv->camera_model;
	if (_tmp3_ > ((guint) gtk_tree_model_iter_n_children ((GtkTreeModel*) _tmp4_, NULL))) {
		GPtrArray* _tmp5_;
		GPtrArray* _tmp6_;
		guint _tmp7_;
		void* _tmp8_;
		CheeseCameraDevice* _tmp9_;
		CheeseCameraDevice* _tmp10_;
		_tmp5_ = devices;
		_tmp6_ = devices;
		_tmp7_ = _tmp6_->len;
		_tmp8_ = g_ptr_array_index (_tmp5_, _tmp7_ - 1);
		_tmp9_ = _g_object_ref0 (G_TYPE_CHECK_INSTANCE_CAST (_tmp8_, CHEESE_TYPE_CAMERA_DEVICE, CheeseCameraDevice));
		_g_object_unref0 (dev);
		dev = _tmp9_;
		_tmp10_ = dev;
		cheese_preferences_dialog_add_camera_device (self, _tmp10_);
	} else {
		Block2Data* _data2_;
		...
		_tmp11_ = self->priv->camera_model;
		gtk_tree_model_get_iter_first ((GtkTreeModel*) _tmp11_, &_tmp12_);
		_data2_->iter = _tmp12_;
		_tmp13_ = self->priv->source_combo;
		gtk_combo_box_get_active_iter (_tmp13_, &_tmp14_);
		active_iter = _tmp14_;
		_tmp15_ = self->priv->camera_model;
		_tmp16_ = active_iter;
		gtk_tree_model_get ((GtkTreeModel*) _tmp15_, &_tmp16_, 1, &_data2_->active_device, -1, -1);
		_data2_->device_removed = FALSE;
		_tmp17_ = devices;
		g_ptr_array_foreach (_tmp17_, ____lambda6__gfunc, _data2_);
		_tmp18_ = _data2_->device_removed;
		if (!_tmp18_) {
			CheeseCameraDevice* old_device = NULL;
			...
			_tmp19_ = self->priv->camera_model;
			_tmp20_ = _data2_->iter;
			gtk_tree_model_get ((GtkTreeModel*) _tmp19_, &_tmp20_, 1, &old_device, -1, -1);
			_tmp21_ = _data2_->iter;
			_tmp22_ = old_device;
			_tmp23_ = _data2_->active_device;
			cheese_preferences_dialog_remove_camera_device (self, &_tmp21_, _tmp22_, _tmp23_);
			_g_object_unref0 (old_device);
		}
	...
	_tmp26_ = cheese_camera_get_selected_device (_tmp25_);
	_tmp27_ = cheese_camera_device_get_name (_tmp26_);
	g_settings_set_string (_tmp24_, "camera", _tmp27_);
	_tmp28_ = self->priv->camera;
	_tmp29_ = cheese_camera_get_selected_device (_tmp28_);
	cheese_preferences_dialog_setup_resolutions_for_device (self, _tmp29_);
	_g_object_unref0 (dev);
}
                                             ...
                                             void* device)
{
	...
	iter = _tmp2_;
	_tmp3_ = self->priv->camera_model;
	_tmp4_ = iter;
	_tmp5_ = dev;
	_tmp6_ = cheese_camera_device_get_name (_tmp5_);
	_tmp7_ = dev;
	gtk_list_store_set (_tmp3_, &_tmp4_, 0, _tmp6_, 1, _tmp7_, -1);
	_tmp8_ = self->priv->camera;
	_tmp9_ = cheese_camera_get_selected_device (_tmp8_);
	_tmp10_ = dev;
	if (_tmp9_ == _tmp10_) {
		GtkComboBox* _tmp11_;
		GtkTreeIter _tmp12_;
	...
	if (gtk_tree_model_iter_n_children ((GtkTreeModel*) _tmp13_, NULL) > 1) {
		GtkComboBox* _tmp14_;
                                                ...
                                                CheeseCameraDevice* active_device_node)
{
	...
	if (device_node == active_device_node) {
		...
		if (_tmp3_ > ((guint) 0)) {
			GtkTreeIter _tmp4_;
			_tmp4_ = *iter;
			cheese_preferences_dialog_set_new_available_camera_device (self, &_tmp4_);
		} else {
			gtk_widget_hide ((GtkWidget*) self);
		}
	}
	_tmp5_ = self->priv->camera_model;
	gtk_list_store_remove (_tmp5_, iter);
	_tmp6_ = self->priv->camera_model;
	if (gtk_tree_model_iter_n_children ((GtkTreeModel*) _tmp6_, NULL) <= 1) {
		GtkComboBox* _tmp7_;
