/* cheese-application.c generated by valac 0.36.1.54-17500, the Vala compiler
...
enum  {
	...
	CHEESE_APPLICATION_LAST_PROPERTY
};
static GParamSpec* cheese_application_properties[CHEESE_APPLICATION_LAST_PROPERTY];
...
void cheese_application_setup_camera (CheeseApplication* self) {
	...
	self->priv->camera = _tmp17_;
	{
		...
		cheese_camera_setup (_tmp18_, NULL, &_inner_error_);
	...
	__catch0_g_error:
	{
		...
		GError* _tmp20_;
		const gchar* _tmp21_;
		CheeseMainWindow* _tmp22_;
		GError* _tmp23_;
		const gchar* _tmp24_;
		err = _inner_error_;
		_inner_error_ = NULL;
		_tmp19_ = video_preview;
		...
		cheese_main_window_show_error (_tmp22_, _tmp24_);
		_g_error_free0 (err);
	...
	g_signal_connect_object (_tmp45_, "state-flags-changed", (GCallback) _cheese_application_on_camera_state_flags_changed_cheese_camera_state_flags_changed, self, 0);
	_tmp46_ = cheese_application_main_window;
...
static void cheese_application_on_camera_state_flags_changed (CheeseApplication* self, GstState new_state) {
	...
	switch (_tmp6_) {
		case GST_STATE_PLAYING:
		{
			...
			_tmp20_ = gtk_application_inhibit ((GtkApplication*) self, (GtkWindow*) _tmp19_, GTK_APPLICATION_INHIBIT_SWITCH | GTK_APPLICATION_INHIBIT_IDLE, _ ("Webcam in use"));
			self->priv->inhibited = _tmp20_;
			break;
		}
		case GST_STATE_NULL:
		{
			...
			_tmp24_ = cheese_application_main_window;
			cheese_main_window_camera_state_change_null (_tmp24_);
			_tmp25_ = self->priv->inhibited;
			if (_tmp25_ != ((guint) 0)) {
				guint _tmp26_;
				_tmp26_ = self->priv->inhibited;
				gtk_application_uninhibit ((GtkApplication*) self, _tmp26_);
			}
			break;
		}
		default:
