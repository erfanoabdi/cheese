/* cheese-window.c generated by valac 0.36.1.54-17500, the Vala compiler
...
enum  {
	...
	CHEESE_MAIN_WINDOW_LAST_PROPERTY
};
static GParamSpec* cheese_main_window_properties[CHEESE_MAIN_WINDOW_LAST_PROPERTY];
...
static void cheese_main_window_on_file_open (CheeseMainWindow* self) {
	...
	__catch4_g_error:
	{
		...
		_inner_error_ = NULL;
		_tmp12_ = filename;
		_tmp13_ = (GtkMessageDialog*) gtk_message_dialog_new ((GtkWindow*) self, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, _ ("Could not open %s"), _tmp12_);
...
static void cheese_main_window_on_file_delete (CheeseMainWindow* self) {
	...
	_tmp4_ = files_length;
	_tmp5_ = files_length;
	_tmp6_ = (GtkMessageDialog*) gtk_message_dialog_new_with_markup ((GtkWindow*) self, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_WARNING, GTK_BUTTONS_NONE, ngettext ("Are you sure you want to permanently delete the file?", "Are you sure you want to permanently delete %d files?", (gulong) _tmp4_), _tmp5_);
	g_object_ref_sink (_tmp6_);
	confirmation_dialog = _tmp6_;
	...
	gtk_dialog_add_button ((GtkDialog*) _tmp8_, _ ("_Delete"), (gint) GTK_RESPONSE_ACCEPT);
	_tmp9_ = confirmation_dialog;
	_tmp10_ = files_length;
	gtk_message_dialog_format_secondary_text (_tmp9_, "%s", ngettext ("If you delete an item, it will be permanently lost", "If you delete the items, they will be permanently lost", (gulong) _tmp10_));
	...
	if (_tmp13_ == ((gint) GTK_RESPONSE_ACCEPT)) {
		...
		_tmp14_ = files;
		{
			...
			GList* file_it = NULL;
			file_collection = _tmp14_;
			for (file_it = file_collection; file_it != NULL; file_it = file_it->next) {
				GFile* _tmp15_;
				GFile* file = NULL;
				_tmp15_ = _g_object_ref0 ((GFile*) file_it->data);
				file = _tmp15_;
				{
					GFile* _tmp16_;
					_tmp16_ = file;
					...
					__catch5_g_error:
					{
						...
						g_warning ("cheese-window.vala:285: Unable to delete file: %s", _tmp19_);
						_tmp20_ = skip_all_errors;
						if (!_tmp20_) {
							GtkMessageDialog* error_dialog = NULL;
							GFile* _tmp21_;
							gchar* _tmp22_;
							...
							_tmp23_ = _tmp22_;
							_tmp24_ = (GtkMessageDialog*) gtk_message_dialog_new ((GtkWindow*) self, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_NONE, _ ("Could not delete %s"), _tmp23_);
							g_object_ref_sink (_tmp24_);
							_tmp25_ = _tmp24_;
							_g_free0 (_tmp23_);
							error_dialog = _tmp25_;
							_tmp26_ = error_dialog;
							gtk_dialog_add_button ((GtkDialog*) _tmp26_, _ ("_Cancel"), (gint) GTK_RESPONSE_CANCEL);
							_tmp27_ = error_dialog;
							gtk_dialog_add_button ((GtkDialog*) _tmp27_, _ ("Skip"), (gint) CHEESE_MAIN_WINDOW_DELETE_RESPONSE_SKIP);
							_tmp28_ = error_dialog;
							gtk_dialog_add_button ((GtkDialog*) _tmp28_, _ ("Skip all"), (gint) CHEESE_MAIN_WINDOW_DELETE_RESPONSE_SKIP_ALL);
							_tmp29_ = error_dialog;
							_tmp30_ = gtk_dialog_run ((GtkDialog*) _tmp29_);
							error_response = _tmp30_;
							_tmp31_ = error_response;
							if (_tmp31_ == ((gint) CHEESE_MAIN_WINDOW_DELETE_RESPONSE_SKIP_ALL)) {
								skip_all_errors = TRUE;
							} else {
								gint _tmp32_;
								_tmp32_ = error_response;
								if (_tmp32_ == ((gint) GTK_RESPONSE_CANCEL)) {
									_g_object_unref0 (error_dialog);
									_g_error_free0 (err);
									_g_object_unref0 (file);
									break;
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				__catch6_g_error:
				{
					...
					_tmp16_ = _tmp15_;
					_tmp17_ = (GtkMessageDialog*) gtk_message_dialog_new ((GtkWindow*) self, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, _ ("Could not move %s to trash"), _tmp16_);
...
static void cheese_main_window_on_file_saveas (CheeseMainWindow* self) {
	...
	if (_tmp14_ == ((gint) GTK_RESPONSE_ACCEPT)) {
		...
		dest = _tmp20_;
		{
			...
			_tmp21_ = src;
			_tmp22_ = dest;
			g_file_copy (_tmp21_, _tmp22_, G_FILE_COPY_OVERWRITE, NULL, NULL, NULL, &_inner_error_);
			if (G_UNLIKELY (_inner_error_ != NULL)) {
		...
		__catch7_g_error:
		{
			...
			err = _inner_error_;
			_inner_error_ = NULL;
			_tmp23_ = target_filename;
			_tmp24_ = (GtkMessageDialog*) gtk_message_dialog_new ((GtkWindow*) self, GTK_DIALOG_MODAL | GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, _ ("Could not save %s"), _tmp23_);
			g_object_ref_sink (_tmp24_);
...
void cheese_main_window_set_wide_mode (CheeseMainWindow* self, gboolean wide_mode) {
	...
	GtkRequisition _tmp37_;
	gint _tmp38_;
	GtkRequisition _tmp39_;
	gint _tmp40_;
...
static void cheese_main_window_finish_countdown_callback (CheeseMainWindow* self) {
	...
	if (_tmp0_ == FALSE) {
		...
		ca_gtk_play_for_widget (_tmp7_, (guint32) 0, CA_PROP_EVENT_ID, "camera-shutter", CA_PROP_MEDIA_ROLE, "event", CA_PROP_EVENT_DESCRIPTION, _ ("Shutter sound"), NULL, NULL);
		_tmp8_ = self->priv->camera;
		_tmp9_ = file_name;
		cheese_camera_take_photo (_tmp8_, _tmp9_);
		_g_free0 (file_name);
	}
	_tmp10_ = self->priv->current_mode;
	if (_tmp10_ == CHEESE_MEDIA_MODE_PHOTO) {
...
void cheese_main_window_toggle_video_recording (CheeseMainWindow* self, gboolean is_start) {
	...
	if (_tmp0_) {
		...
		gtk_widget_set_tooltip_text ((GtkWidget*) _tmp6_, _ ("Stop recording"));
		_tmp7_ = self->priv->take_action_button_image;
		gtk_image_set_from_icon_name (_tmp7_, "media-playback-stop-symbolic", GTK_ICON_SIZE_BUTTON);
		...
		GtkImage* _tmp11_;
		_tmp8_ = self->priv->camera;
		cheese_camera_stop_video_recording (_tmp8_);
		_tmp9_ = self->priv->timeout_layer;
		clutter_text_set_text (_tmp9_, "00:00:00");
		_tmp10_ = self->priv->take_action_button;
		gtk_widget_set_tooltip_text ((GtkWidget*) _tmp10_, _ ("Record a video"));
		_tmp11_ = self->priv->take_action_button_image;
		gtk_image_set_from_icon_name (_tmp11_, "camera-web-symbolic", GTK_ICON_SIZE_BUTTON);
...
void cheese_main_window_toggle_photo_bursting (CheeseMainWindow* self, gboolean is_start) {
	...
	if (_tmp0_) {
		...
		gtk_widget_set_tooltip_text ((GtkWidget*) _tmp1_, _ ("Stop taking pictures"));
		cheese_main_window_burst_take_photo (self);
		_tmp2_ = self->priv->settings;
		_tmp3_ = g_settings_get_int (_tmp2_, "burst-delay");
		burst_delay = _tmp3_;
		_tmp4_ = self->priv->settings;
		_tmp5_ = g_settings_get_int (_tmp4_, "countdown-duration");
		countdown_duration = 500 + (_tmp5_ * 1000);
		_tmp7_ = burst_delay;
		_tmp8_ = countdown_duration;
		if ((_tmp7_ - _tmp8_) < 1000) {
			GSettings* _tmp9_;
			gboolean _tmp10_;
			_tmp9_ = self->priv->settings;
			_tmp10_ = g_settings_get_boolean (_tmp9_, "countdown");
			_tmp6_ = _tmp10_;
		} else {
			_tmp6_ = FALSE;
		}
		if (_tmp6_) {
			gint _tmp11_;
			guint _tmp12_;
			_tmp11_ = countdown_duration;
			_tmp12_ = g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) _tmp11_, _cheese_main_window_burst_take_photo_gsource_func, g_object_ref (self), g_object_unref);
			self->priv->burst_callback_id = _tmp12_;
		} else {
			gint _tmp13_;
			guint _tmp14_;
			_tmp13_ = burst_delay;
			_tmp14_ = g_timeout_add_full (G_PRIORITY_DEFAULT, (guint) _tmp13_, _cheese_main_window_burst_take_photo_gsource_func, g_object_ref (self), g_object_unref);
			self->priv->burst_callback_id = _tmp14_;
		}
	} else {
		gboolean _tmp15_ = FALSE;
		CheeseCountdown* _tmp16_;
		GtkButton* _tmp20_;
		CheeseFileUtil* _tmp21_;
		guint _tmp22_;
		_tmp16_ = self->priv->current_countdown;
		if (_tmp16_ != NULL) {
			CheeseCountdown* _tmp17_;
			gboolean _tmp18_;
			_tmp17_ = self->priv->current_countdown;
			_tmp18_ = _tmp17_->running;
			_tmp15_ = _tmp18_;
		} else {
			_tmp15_ = FALSE;
		}
		...
		cheese_main_window_enable_mode_change (self);
		_tmp20_ = self->priv->take_action_button;
		gtk_widget_set_tooltip_text ((GtkWidget*) _tmp20_, _ ("Take multiple photos"));
		self->priv->burst_count = 0;
		_tmp21_ = self->priv->fileutil;
		cheese_fileutil_reset_burst (_tmp21_);
		_tmp22_ = self->priv->burst_callback_id;
...
static void cheese_main_window_toggle_effects_selector (CheeseMainWindow* self, gboolean active) {
	...
	if (_tmp2_ == ((guint) 0)) {
		...
		if (_tmp5_) {
			...
			_tmp8_ = self->priv->current_effects_page;
			cheese_main_window_activate_effects_page (self, (gint) _tmp8_);
		} else {
			ClutterActor* _tmp9_;
	...
	_tmp12_ = active;
	cheese_camera_toggle_effects_pipeline (_tmp11_, _tmp12_);
	cheese_main_window_setup_effects_page_switch_sensitivity (self);
		...
		_tmp7_ = _tmp6_->effects;
		{
			...
			for (effect_it = effect_collection; effect_it != NULL; effect_it = effect_it->next) {
				...
				effect = _tmp8_;
				{
					...
					ClutterGridLayout* _tmp9_;
					ClutterActor* grid = NULL;
					ClutterActor* _tmp10_;
					ClutterActor* _tmp11_;
					ClutterGridLayout* _tmp12_;
					ClutterActor* _tmp13_;
					ClutterActor* _tmp14_;
					ClutterGridLayout* _tmp15_;
					ClutterGridLayout* _tmp16_;
		...
		_tmp18_ = _tmp17_->effects;
		{
			...
			for (effect_it = effect_collection; effect_it != NULL; effect_it = effect_it->next) {
				...
				effect = _tmp19_;
				{
					...
					ClutterTapAction* _tmp36_;
					ClutterActor* _tmp37_;
					ClutterTapAction* _tmp38_;
					ClutterTapAction* _tmp39_;
					...
					ClutterLayoutManager* _tmp72_;
					ClutterGridLayout* _tmp73_;
					ClutterGridLayout* _tmp74_;
...
void cheese_main_window_set_current_mode (CheeseMainWindow* self, CheeseMediaMode mode) {
	...
	switch (_tmp3_) {
		case CHEESE_MEDIA_MODE_PHOTO:
		{
			...
			gtk_widget_set_tooltip_text ((GtkWidget*) _tmp4_, _ ("Take a photo using a webcam"));
			break;
		}
		case CHEESE_MEDIA_MODE_VIDEO:
		{
			...
			gtk_widget_set_tooltip_text ((GtkWidget*) _tmp5_, _ ("Record a video using a webcam"));
			_tmp6_ = self->priv->timeout_layer;
			clutter_text_set_text (_tmp6_, "00:00:00");
		...
		case CHEESE_MEDIA_MODE_BURST:
		{
			...
			gtk_widget_set_tooltip_text ((GtkWidget*) _tmp8_, _ ("Take multiple photos using a webcam"));
			break;
		}
		default:
		break;
